...
#include "../qct.inc"
  ...
  private
  public    ::    N3_UMN_PES_Type


  Type, extends(PES_Type) :: N3_UMN_PES_Type
    real(rkp) ::  shiftof0
  contains
    procedure ::  Initialize     =>    Initialize_N3_UMN_PES
    procedure ::  Output         =>    Output_N3_UMN_PES
    procedure ::  Compute        =>    Compute_N3_UMN_PES_1d
    procedure ::  Potential      =>    N3_UMN_Potential_From_R
    procedure ::  TriatPotential =>    N3_UMN_Potential_From_R_OnlyTriat
  End Type

  logical                 ,parameter      :: i_Debug_Global = .False.

  real(rkp) ,dimension(0:111)   :: rM       ! Array to store monomials
  real(rkp) ,dimension(0:305)   :: P        ! Array to store polynomials
  real(rkp) ,dimension(276)     :: B        ! Array to store basis functions
  real(rkp) ,dimension(6,0:111) :: dMdR     ! The derivative of monomials w.r.t. R
  real(rkp) ,dimension(6,0:305) :: dPdR     ! The derivative of basis functions w.r.t. R
  real(rkp) ,dimension(6,276)   :: dBdR     ! The derivative of B w.r.t. R 

  ! Nonlinear parameters:
  real(rkp)                 ,parameter :: a  = 1.0d0    ! a(in Ang)
  real(rkp)                 ,parameter :: ab = 1.5d0    ! ab (in Ang^2)
  real(rkp)                 ,parameter :: re = 1.098d0  ! re (in Ang)

  ! Reference energy of infinitely separated N2 + N2 in hartree
  real(rkp)                 ,parameter :: Eref = Zero!-218.40801323d0

  ! For N2 + N2 framework total diss. energy is 2De 2*228.7 kcal/mol 
  real(rkp)                 ,parameter :: totdiss = 457.4d0 

  ! Linear parameters optimized by the weighted-least square fitting
  real(rkp) ,dimension(276) ,parameter :: C = (/ &
...
Subroutine Initialize_N3_UMN_PES( This, Input, Atoms, iPES, i_Debug )
  ...
  integer                                                   ::    iP
  character(*)                    ,parameter                ::    Name_PES = 'N3_UMN'
  real(rkp)                                                 ::    Temp
  real(rkp)                                                 ::    veccsd
  real(rkp)                                                 ::    veuse
  ...
  if (i_Debug_Loc) call Logger%Entering( "Initialize_N3_UMN_PES" )
...
Subroutine Output_N3_UMN_PES( This, Unit )

  class(N3_UMN_PES_Type)                  ,intent(in)     ::    This
...
!________________________________________________________________________________________________________________________________!
Function N3_UMN_Potential_From_R( This, R, Q ) result( V )
  ...
  RTemp(2) = R(2)
  RTemp(4) = R(3)  
  call n4pes(RTemp * B_To_Ang, VTriat, dVdRTriat, 0)
  V        = VDiat + VTriat 
...
!________________________________________________________________________________________________________________________________!
Function N3_UMN_Potential_From_R_OnlyTriat( This, R, Q ) result( V )
  ...
  RTemp(2) = R(2)
  RTemp(4) = R(3)  
  call n4pes(RTemp * B_To_Ang, V, dVdR, 0)
...
!________________________________________________________________________________________________________________________________!
Subroutine Compute_N3_UMN_PES_1d( This, R, Q, V, dVdR, dVdQ )
  ...
  RTemp(2) = R(2)
  RTemp(4) = R(3)
  call n4pes(RTemp * B_To_Ang, VTriat, dVdRTriat, 1)
  dVdRTriat  = dVdRTriat

  V       = VDiat       + VTriat
  dVdR(1) = dVdRDiat(1) + dVdRTriat(1)
  dVdR(2) = dVdRDiat(2) + dVdRTriat(2)
  dVdR(3) = dVdRDiat(3) + dVdRTriat(4)
...
!   !real(rkp) ,dimension(4) ,intent(in)  :: X, Y, Z
!   real(rkp)               ,intent(out) :: E
!   real(rkp) ,dimension(6) ,intent(out) :: dVdR
!   !real(rkp) ,dimension(4) ,intent(out) :: dEdX, dEdY, dEdZ

!   real(rkp)                            :: V
!   real(rkp) ,dimension(12)             :: Xcart
!   real(rkp) ,dimension(12)             :: dVdX
!   integer                              :: i

!   ! ! Convert to local variables
!   ! do i=1,4
!   !   Xcart(3*i-2) = X(i) * B_To_Ang
!   !   Xcart(3*i-1) = Y(i) * B_To_Ang
!   !   Xcart(3*i)   = Z(i) * B_To_Ang
!   ! enddo

!   call n4pes(R, V, dVdR, 1)
!   !call n4pes(Xcart, V, dVdX, 1)

!   ! Convet local output to the ones ANT wants
!   E = V*Kcm_To_Hartree + Eref
!   ! do i=1,4
!   !   dEdX(i) = dVdX(3*i-2) * Kcm_To_Hartree * B_To_Ang
!   !   dEdY(i) = dVdX(3*i-1) * Kcm_To_Hartree * B_To_Ang
!   !   dEdZ(i) = dVdX(3*i)   * Kcm_To_Hartree * B_To_Ang
!   ! enddo
...
!________________________________________________________________________________________________________________________________!
Subroutine n4pes(R, V, dVdR, igrad)
...
!**********************************************************************
  ...
  do i=1,6
    rms(i) = exp( - (R(i) - re)/a - ((R(i)-re)**Two)/ab )
  enddo
...
!**********************************************************************
  ...
  do i=1,6
    dmsdr(i,i) = ( - Two*(R(i) - re)/ab - One/a) * exp(-(r(i)-re)/a - ((r(i)-re)**Two)/ab)
  end do 
